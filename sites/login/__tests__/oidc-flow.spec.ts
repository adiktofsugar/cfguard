...
test.describe('OIDC Provider Flow', () => {
  ...
  test('should show login form on authorization request', async ({ page }) => {
    await page.goto(`/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&state=${state}`);
  ...
  test('should complete full authorization code flow', async ({ page, request, context }) => {
    ...
    await page.goto(`/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(testRedirectUri)}&response_type=code&state=${state}`);
